...
static int hx170dec_ioctl(struct file *filp,
...
{
	unsigned int len;
	unsigned int iopgsize;
	unsigned int pgsize = 1<<PAGE_SHIFT;

	PDEBUG("mmap : start = 0x%08x , end = 0x%08x , pgoff = 0x%08x\n" ,
						vma->vm_start , vma->vm_end , vma->vm_pgoff);

	/* Check the request range , if this is valid */
	if ( vma->vm_pgoff < (((unsigned int)hx170dec_data.iobaseaddr) >> PAGE_SHIFT ) ||
		vma->vm_pgoff > (((unsigned int)hx170dec_data.iobaseaddr + hx170dec_data.iosize ) >> PAGE_SHIFT ))
	{
		PDEBUG( "max = 0x%08x, min = 0x%08x\n" , 
					((unsigned int)hx170dec_data.iobaseaddr) >> PAGE_SHIFT ,
					((unsigned int)hx170dec_data.iobaseaddr + hx170dec_data.iosize ) >> PAGE_SHIFT);
		return -EINVAL;
	}

	len = vma->vm_end - vma->vm_start;
	iopgsize = ( ( hx170dec_data.iosize + pgsize - 1)/pgsize )*pgsize;
	
	if ( ( vma->vm_pgoff << PAGE_SHIFT ) + len >
		(unsigned int)hx170dec_data.iobaseaddr + iopgsize )
	{
		PDEBUG( " vma : %08x , hx170: %08x\n " , ( vma->vm_pgoff << PAGE_SHIFT ) + len ,
								(unsigned int)hx170dec_data.iobaseaddr + hx170dec_data.iosize);
		return -EINVAL;
	}
...
int __init hx170dec_init(void)
{
    ...
    hx170dec_class = class_create(THIS_MODULE , "hx170dec_class");
    ...
    device_create( hx170dec_class, NULL, MKDEV( hx170dec_major , 0), NULL , "hx170" );
...
static int ReserveIO(void)
{
    irq_set_irq_type(AT91SAM9G45_ID_VDEC, IRQ_TYPE_LEVEL_HIGH);
